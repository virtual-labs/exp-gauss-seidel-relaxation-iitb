function activity2() {
    ...
    <div style='margin-top: 2vw;'>
        ...
        <div class="row">
            <div class="col-6">
                Enter relaxation factor between 0 and 1
            </div>
            <div class="col-6">
                <input type='text' id='lambda_inp' class='form-control'>
            </div>
        </div>
        <br>

        <button class='btn btn-info std-btn' style='position: relative; left: 50vw;' onclick='verify_relaxation();' id='temp-btn-2' >Next</button>
...
function verify_relaxation() {
    let a = (document.getElementById('lambda_inp'));
    console.log('Relax value: ', a.value);
    lambda = parseFloat(a.value);
    if (lambda >= 0 && lambda <= 1) {
        start_act2();
    }
    else {
        alert('Enter the correct relaxation value');
    }
}
function start_act2() {
    ...
    <div class='collapse divide' style='style='margin-top: 2vw; 'width: 80%; margin: auto;' id='tb2-box'>
        ...
        <h5>Consider the values upto 4 decimal places. Start with initial guess solution [0 0 0]T. Perform Gauss Seidel iterations</h5>
